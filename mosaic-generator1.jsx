...
import { Upload, Download, Loader2, Square, Circle, Hexagon, Triangle, Diamond, Palette, PenTool } from "lucide-react";
    ...
    <div style={{ background: PAPER, minHeight: "100%", color: INK, fontFamily: "ui-sans-serif, system-ui, sans-serif" }} className="w-full flex flex-col lg:flex-row gap-0">
      ...
      <div style={{ background: PANEL, borderRight: `1px solid ${LINE}` }} className="lg:w-72 w-full flex-shrink-0 p-5 flex flex-col gap-6">
        <div>
          ...
          <div style={{ fontSize: 20, fontWeight: 800, letterSpacing: "-0.01em" }}>
            {appMode === "mosaic" ? "Image → Color-by-Number" : "Image → Coloring Page"}
          </div>
        </div>

        <div style={{ border: `1px solid ${LINE}` }} className="flex rounded-sm overflow-hidden">
          {[
            { key: "mosaic", label: "Mosaic", Icon: Palette },
            { key: "coloring", label: "Coloring Book", Icon: PenTool },
          ].map(({ key, label, Icon }) => (
            <button key={key} onClick={() => setAppMode(key)}
              style={{ flex: 1, padding: "9px 0", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, fontSize: 12, fontWeight: 600, background: appMode === key ? MUSTARD : "transparent" }}>
              <Icon size={14} /> {label}
            </button>
          ))}
        </div>
        ...
        {imageSrc && appMode === "coloring" && (
          <>
            <div className="flex flex-col gap-2">
              <label style={{ fontSize: 11, letterSpacing: "0.08em" }} className="uppercase font-semibold flex justify-between">
                <span>Detail</span>
                <span style={{ fontFamily: "ui-monospace, monospace", color: TEAL }}>{cbDetail}px</span>
              </label>
              <input type="range" min={300} max={1400} step={50} value={cbDetail} onChange={(e) => setCbDetail(Number(e.target.value))} />
              <div style={{ fontSize: 10, color: "#8A8676" }}>Working resolution — higher catches finer detail but is slower.</div>
            </div>

            <div className="flex flex-col gap-2">
              <label style={{ fontSize: 11, letterSpacing: "0.08em" }} className="uppercase font-semibold flex justify-between">
                <span>Sensitivity</span>
                <span style={{ fontFamily: "ui-monospace, monospace", color: TEAL }}>{cbThreshold}</span>
              </label>
              <input type="range" min={15} max={180} step={1} value={cbThreshold} onChange={(e) => setCbThreshold(Number(e.target.value))} />
              <div style={{ fontSize: 10, color: "#8A8676" }}>Lower = more lines picked up. Higher = only strong edges.</div>
            </div>

            <div className="flex flex-col gap-2">
              <label style={{ fontSize: 11, letterSpacing: "0.08em" }} className="uppercase font-semibold flex justify-between">
                <span>Smoothing</span>
                <span style={{ fontFamily: "ui-monospace, monospace", color: TEAL }}>{cbBlur}</span>
              </label>
              <input type="range" min={0} max={4} step={1} value={cbBlur} onChange={(e) => setCbBlur(Number(e.target.value))} />
              <div style={{ fontSize: 10, color: "#8A8676" }}>Reduces noise/grain before edge detection.</div>
            </div>

            <div className="flex flex-col gap-2">
              <label style={{ fontSize: 11, letterSpacing: "0.08em" }} className="uppercase font-semibold flex justify-between">
                <span>Line thickness</span>
                <span style={{ fontFamily: "ui-monospace, monospace", color: TEAL }}>{cbThickness}</span>
              </label>
              <input type="range" min={0} max={3} step={1} value={cbThickness} onChange={(e) => setCbThickness(Number(e.target.value))} />
            </div>

            <button onClick={downloadColoringPng} disabled={!cbResult}
              style={{ border: `1px solid ${INK}`, color: INK }}
              className="w-full flex items-center justify-center gap-2 py-2.5 rounded-sm text-sm font-semibold hover:bg-black/5 transition disabled:opacity-40">
            ...
            {cbResult && (
              <div style={{ fontSize: 11, color: "#6B6B60", lineHeight: 1.6 }}>
                {cbResult.width} × {cbResult.height}px outline
              </div>
            )}
            <div style={{ fontSize: 10, color: "#8A8676" }}>
              Pure edge-detection, no AI — SVG/vector export isn't available yet for this mode.
            </div>
          </>
        )}
      </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {!imageSrc ? (
          ...
          <div className="flex-1 overflow-auto p-6 flex items-start justify-center" style={{ background: "#f2f2f0" }}>
            <div className="relative inline-block">
              {cbProcessing && (
                <div className="absolute inset-0 flex items-center justify-center bg-white/60 z-10">
                  <Loader2 className="animate-spin" style={{ color: TEAL }} size={28} />
                </div>
              )}
              {cbResult && (
                <img src={cbResult.dataUrl} alt="Coloring book outline" style={{ display: "block", maxWidth: "100%", boxShadow: `0 0 0 1px ${LINE}`, background: "#fff" }} />
              )}
            </div>
          </div>
        ) : (
